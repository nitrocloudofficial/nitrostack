...
describe("Analytics Service Tests", () => {
    ...
    beforeEach(() => {
        ...
        mockDataService = {
            ...
            queryReadings: vi.fn(async (deviceId, metrics, startTs, endTs) => {
                return [
        ...
        analyticsService = new TelemetryAnalyticsService(mockDataService);
    });
    ...
    it("should generate CSV training dataset for multiple devices and metrics", async () => {
        const result = await analyticsService.createTrainingDataset(["device-1", "device-2"], ["temp", "humidity"], 1000, 2000, "CSV");
    ...
    it("should generate JSON training dataset for multiple devices and metrics", async () => {
        const result = await analyticsService.createTrainingDataset(["device-1", "device-2"], ["temp", "humidity"], 1000, 2000, "JSON");
        expect(result.metadata.rowCount).toBe(4);
    ...
    it("should export CSV telemetry file and return filename and row count", async () => {
        const result = await analyticsService.exportTelemetryCSV(["device-1", "device-2"], ["temp", "humidity"], 1000, 2000);
    ...
    it("should return empty datasets when no telemetry exists", async () => {
        const result = await analyticsService.createTrainingDataset(["empty-device"], ["temp"], 1000, 2000, "JSON");
