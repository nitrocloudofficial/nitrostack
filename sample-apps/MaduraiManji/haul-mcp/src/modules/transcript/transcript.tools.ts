...
Splits the content into tasks, calendar events, risks, and progress checks, and automatically saves them into the respective agent databases.`,
  ...
  async analyzeTranscript(input: any, ctx: ExecutionContext) {
...
`;
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (lower.includes('task') || lower.includes('todo') || lower.includes('complete') || lower.includes('assign') || lower.includes('finish')) {
          let owner = 'Team';
          if (lower.includes('aksha')) owner = 'Aksha';
          else if (lower.includes('punith')) owner = 'Punith';
          else if (lower.includes('developer')) owner = 'Developer';

          let deadline = 'End of hackathon';
          if (lower.includes('tomorrow')) deadline = 'Tomorrow';
          else if (lower.includes('friday')) deadline = 'Friday';
          else if (lower.includes('wednesday')) deadline = 'Wednesday';
      ...
      for (const t of resultJson.tasks) {
        await new Promise<void>((resolve) => {
          db.run(
            ...
            [t.title, t.owner, t.deadline, 'active'],
            () => resolve()
          );
        });
      ...
      for (const ev of resultJson.events) {
        await new Promise<void>((resolve) => {
          db.run(
            ...
            [ev.summary, ev.description, ev.startTime, ev.endTime, 'upcoming'],
            () => resolve()
          );
        });
      ...
      for (const r of resultJson.risks) {
        ...
        await new Promise<void>((resolve) => {
          db.run(
            ...
            [r.dependency, r.status, riskLevel, `Risk analysis on ${r.dependency}: ${r.status}`, suggestion],
            () => resolve()
          );
        });
      ...
      for (const p of resultJson.progress) {
        await new Promise<void>((resolve) => {
          db.run(
            ...
            [p.taskName, p.status, `Progress check: ${p.status}`, p.action, p.suggestion],
            () => resolve()
          );
        });
  ...
  async exportTasksPdf(_input: any, ctx: ExecutionContext) {
    return new Promise((resolve) => {
      db.all(`SELECT * FROM tasks ORDER BY created_at DESC`, (err: Error | null, rows: any[]) => {
        if (err) {
