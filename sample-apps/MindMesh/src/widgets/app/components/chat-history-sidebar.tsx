...
import { DESIGN, PHASE_GROUPS, getPhaseColor, getPhaseInfo, ALL_PHASES } from './design-tokens';
import { PhaseBadge, ToolChip, StatusDot, StatusIndicator, Card, Input, Button, Badge, Separator } from './ui';
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <h2 style={{ fontSize: 13, fontWeight: 700, color: DESIGN.colors.fg, margin: 0, textTransform: 'uppercase', letterSpacing: 0.5, fontFamily: DESIGN.fonts.mono }}>
            Chat History
          </h2>
          <Button size="sm" variant="ghost" onClick={handleNewConversation} style={{ fontSize: 10, padding: '4px 8px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: DESIGN.spacing.xs }}>
          <Button
            variant={state.phaseFilter === null ? 'primary' : 'ghost'}
            size="sm"
          ...
          {PHASE_GROUPS.map(group => (
            <div key={group.groupId} style={{ display: 'flex', flexWrap: 'wrap', gap: DESIGN.spacing.xs }}>
              {group.phases.map(p => (
                <Button
                  key={p.id}
                  variant={state.phaseFilter === p.id ? 'primary' : 'ghost'}
                  size="sm"
                  onClick={() => handlePhaseFilterChange(state.phaseFilter === p.id ? null : p.id)}
                  style={{
                    fontSize: 10,
                    padding: '4px 8px',
