...
> BharatFin is an AI-powered financial intelligence platform built on India's Digital Public Infrastructure.
...
**BharatFin** is an [MCP (Model Context Protocol)](https://nitrostack.ai) server that extends AI assistants — like Claude, Cursor, and any MCP-compatible client — with new, real-world capabilities. It is built and deployed on [Nitrostack](https://nitrostack.ai), the fastest way to build, deploy, and share MCP apps.
...
🚀 **Live MCP endpoint:** https://bharatfin-6a6d91c4-code-catalyst-srmist.app.nitrocloud.ai
...
```json
{
  "mcpServers": {
    "bharatfin": {
      "url": "https://bharatfin-6a6d91c4-code-catalyst-srmist.app.nitrocloud.ai"
    }
  }
}
```
...
`BFSI & FinTech` · `BharatFin` · `MCP` · `Model Context Protocol` · `MCP server` · `MCP app` · `AI tools` · `AI agents` · `LLM tools` · `Claude MCP` · `Nitrostack` · `deploy MCP server` · `build MCP app`
