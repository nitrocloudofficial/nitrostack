...
export default function LookupSellerSupportWidget() {
  const theme = useTheme();
  const { isReady, getToolOutput, openExternal } = useWidgetSDK();
  const toolData = getToolOutput<SupportData>();
  ...
  const pages = toolData.data?.supportPages || [];
      ...
      <div style={{
        ...
        <div>
          <h2 style={{ margin: '0 0 4px 0', fontSize: '18px', fontWeight: '600' }}>
            Seller Support Lookup
          </h2>
          <p style={{ margin: '0', fontSize: '14px', color: subTextColor }}>
            Found {toolData.data?.count || 0} support pages
          </p>
        </div>
      </div>

      {pages.length === 0 ? (
        <p style={{ color: subTextColor }}>No support pages found.</p>
      ) : (
