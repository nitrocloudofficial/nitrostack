import type { RouterOutput, DraftedMessage } from "./types.js";
 ...
 */
export async function processRouterOutput(
    output: RouterOutput
): Promise<void> {
 ...
 */
export function formatForWidget(output: RouterOutput): any {
    return {
 ...
 */
export function summarizeOutput(output: RouterOutput): string {
    const parts: string[] = [];

    if (output.schedulingResult) {
        parts.push(
            `Meeting scheduled: ${output.schedulingResult.time} (${output.schedulingResult.attendees.length} attendees, ${output.schedulingResult.confidence} confidence)`
        );
    }

    if (output.delegationResult) {
        parts.push(
            `Task assigned: ${output.delegationResult.taskId} → ${output.delegationResult.owner} (due ${output.delegationResult.deadline})`
        );
    }
