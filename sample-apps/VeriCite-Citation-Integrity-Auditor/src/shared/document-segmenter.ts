 ...
 */
const REFERENCE_HEADING_SOURCE =
    String.raw`^[ \t]*(?:#{1,6}[ \t]*)?(?:\d+[.)][ \t]*)?(?:references|bibliography|works[ \t]+cited|literature[ \t]+cited|reference[ \t]+list)[ \t]*:?[ \t]*$`;
 ...
 * bibliography.
 */
const TERMINAL_HEADING_SOURCE =
    String.raw`^[ \t]*(?:#{1,6}[ \t]*)?(?:\d+[.)][ \t]*)?(?:appendix(?:[ \t]+[a-z0-9]+)?|appendices|acknowledgements?|acknowledgments?|author[ \t]+contributions?|funding(?:[ \t]+statement)?|supplementary(?:[ \t]+(?:material|information))?|conflicts?[ \t]+of[ \t]+interest|declarations?[ \t]+of[ \t]+interest|competing[ \t]+interests?|data[ \t]+availability(?:[ \t]+statement)?|ethics[ \t]+(?:statement|approval))[ \t]*:?[ \t]*$`;
...
const CAPTION_BLOCK =
    /^(?:figure|fig\.?|table|tbl\.?|chart|exhibit|scheme|equation|eq\.?|algorithm|listing|appendix)\s*\.?\s*\d+/i;
...
const KNOWN_HEADING_WORDS =
    /^(?:abstract|introduction|background|related\s+work|methods?|methodology|materials(?:\s+and\s+methods)?|results?|discussion|conclusions?|limitations?|future\s+work|summary|keywords?|contents?|index)\s*:?$/i;
...
export interface DocumentSegments {
    /** The paper's argument: everything before the bibliography/back matter. */
    body: string;

    /** The reference list, or null when the document has none. */
...
export interface BodySentence {
    ...
    /** Zero-based index of the paragraph this sentence came from. */
...
export function isNonContentBlock(block: string): boolean {
    const trimmed = block.trim();
    if (trimmed.length === 0) return true;

    if (NOISE_BLOCK.test(trimmed)) return true;
    if (CAPTION_BLOCK.test(trimmed)) return true;
    if (REFERENCE_ENTRY_BLOCK.test(trimmed)) return true;
    if (KNOWN_HEADING_WORDS.test(trimmed)) return true;
...
export function splitSentences(paragraph: string): string[] {
    if (typeof paragraph !== 'string' || paragraph.trim().length === 0) return [];
 ...
 */
export function extractBodySentences(body: string): BodySentence[] {
    const sentences: BodySentence[] = [];
    const paragraphs = splitParagraphs(body);

    paragraphs.forEach((paragraph, paragraphIndex) => {
        if (isNonContentBlock(paragraph)) return;
