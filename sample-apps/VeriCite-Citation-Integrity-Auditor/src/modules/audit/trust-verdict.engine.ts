...
/**
 * ============================================================
 * VeriCite — Trust Verdict Engine
 * ------------------------------------------------------------
 * Multi-factor decision engine producing an explainable,
 * evidence-backed TrustVerdict.
 *
 * WHAT CHANGED
 *
 * 1. Degeneracy removed. The first classification branch used to
 *    escalate to CRITICAL whenever `missingRatio > 0.5`. Combined
 *    with a mapper bug that marked EVERY claim as missing a
 *    citation, that gate fired on every audit — CRITICAL was the
 *    only reachable level and HIGH/MODERATE/LOW were dead code.
 *    Citation coverage now influences the verdict proportionally,
 *    through the Integrity Score, instead of as a cliff edge.
 *
 * 2. Contradiction is now proportional too. A single contradicted
 *    claim in a long document no longer detonates the verdict; it
 *    caps the ceiling at MODERATE_TRUST and lowers the score. A
 *    material share of contradictions still yields CRITICAL.
 *
 * 3. New `inconclusive` flag. "We could not complete the audit" and
 *    "this document is untrustworthy" are different findings and
 *    were previously indistinguishable. Provider outages now report
 *    honestly instead of being laundered into a damning verdict.
 *
 * 4. Reasoning now cites real evidence — resolved reference counts,
 *    retractions, named contradicting papers — because the mapper
 *    finally passes evidence through.
 *
 * PURE · DETERMINISTIC · EXPLAINABLE · DEFENSIVE
 * ============================================================
 */
...
export class TrustVerdictEngine {
    ...
    static computeVerdict(input: VerdictInput): TrustVerdict {
        ...
        const { level, title } = TrustVerdictEngine.determineLevelAndTitle(
            integrityScore,
            summary,
            ratios,
            inconclusive,
    ...
    private static determineLevelAndTitle(
        score: number,
        summary: AuditSummary,
        ratios: Ratios,
        inconclusive: boolean,
    ): { level: TrustVerdictLevel; title: string } {
    ...
    private static determineLevel(
        ...
        if (
            score < VERDICT_THRESHOLDS.CRITICAL_SCORE
        ...
        if (
            score < VERDICT_THRESHOLDS.LOW_SCORE
        ...
        if (
            score < VERDICT_THRESHOLDS.MODERATE_SCORE
    ...
    private static titleFor(level: TrustVerdictLevel, summary: AuditSummary): string {
        switch (level) {
            case 'CRITICAL':
    ...
    private static reason(
        ...
        if (inconclusive) {
            points.push(
                `COVERAGE WARNING: ${summary.errors} of ${summary.totalClaims} claim(s) could not be checked. `
        ...
        if (summary.totalCitations > 0) {
            points.push(
                `Reference list: ${summary.totalCitations} entries parsed, ${summary.resolvedCitations} confirmed `
    ...
    private static weaknesses(
        ...
        if (summary.contradicted > 0) {
            ...
            weaknesses.push(
                `${summary.contradicted} claim(s) contradicted by published literature`
    ...
    private static computeConfidence(summary: AuditSummary, ratios: Ratios): number {
    ...
    private static noClaimsVerdict(): TrustVerdict {
        return {
            level: 'CRITICAL',
            score: 0,
            title: 'Audit Incomplete: No Extracted Claims',
            summary:
                'No verifiable factual claims were extracted from the document, so citation integrity could not be '
