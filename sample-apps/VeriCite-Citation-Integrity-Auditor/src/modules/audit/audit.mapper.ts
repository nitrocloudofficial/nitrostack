...
/**
 * ============================================================
 * VeriCite — Audit Mapper
 * ------------------------------------------------------------
 * Boundary layer between the stage services and the final report.
 * Owns all derived analytics: summary counts, Integrity Score,
 * severity, and report assembly.
 *
 * WHAT CHANGED
 *
 * The mapper used to be a TRANSLATOR between two incompatible DTO
 * families, and it translated badly:
 *
 *   • It read `citationMarker` (singular) while the extractor
 *     emitted `citationMarkers` (plural array)   -> always ''
 *   • It read `citationId` from a verifier that never set it
 *                                                -> always ''
 *   • `missingCitation` counted `!r.citationId`, so it always
 *     equalled `totalClaims`, driving the score to a clamped 0 and
 *     forcing every verdict to CRITICAL
 *   • It read `metadata` and `existence`, neither of which the
 *     verifier produced -> undefined / 'AMBIGUOUS' on every claim
 *   • It discarded `supportingEvidence` / `contradictingEvidence`
 *     entirely
 *
 * Every stage now emits canonical contract objects, so there is
 * nothing left to translate. The mapper's job narrowed to what it
 * should always have been:
 *
 *   NORMALISE — clamp ranges, default arrays, guarantee invariants
 *   AGGREGATE — summary counts from real data
 *   SCORE     — Integrity Score and severity
 *   ASSEMBLE  — the final AuditReport
 *
 * It no longer renames a single field, and it no longer invents a
 * value for anything a producer failed to supply.
 *
 * PURE · STATIC · SYNCHRONOUS · DEFENSIVE
 * ============================================================
 */
...
export class AuditMapper {
    ...
    static normaliseClaims(claims: Claim[] | undefined): Claim[] {
    ...
    static normaliseCitations(citations: Citation[] | undefined): Citation[] {
    ...
    static normaliseResults(results: VerificationResult[] | undefined): VerificationResult[] {
        if (!Array.isArray(results)) return [];

        return results
            .filter((r): r is VerificationResult => Boolean(r))
            .map((r) => ({
                ...r,
    ...
    private static normaliseEvidence(evidence: EvidenceRecord[] | undefined): EvidenceRecord[] {
    ...
    static buildSummary(
        ...
        const countBy = (status: VerificationStatus): number =>
            safeResults.filter((r) => r.status === status).length;
     ...
     */
    static computeIntegrityScore(summary: AuditSummary): number {
        const scoredCount =
            summary.supported
        ...
        if (scoredCount === 0) return SCORE_MIN;

        const points =
            summary.supported * SCORING.STATUS_POINTS.SUPPORTED
        ...
        const coveragePenalty =
            (1 - AuditMapper.clampUnit(summary.citationCoverage)) * SCORING.MAX_COVERAGE_PENALTY;

        const retractionPenalty = Math.min(
            SCORING.MAX_RETRACTION_PENALTY,
            summary.retractedCitations * SCORING.RETRACTION_PENALTY_PER_CITATION,
        );
    ...
    static scoreToSeverity(score: number): ReportSeverity {
        if (score >= SEVERITY_THRESHOLDS.GREEN) return 'GREEN';
        if (score >= SEVERITY_THRESHOLDS.AMBER) return 'AMBER';
        return 'RED';
    }
    ...
    static buildReport(input: BuildReportInput): AuditReport {
        ...
        const verdict: TrustVerdict = TrustVerdictEngine.computeVerdict({
            integrityScore,
            summary,
            results,
            citations,
    ...
    private static clampUnit(value: number | undefined): number {
