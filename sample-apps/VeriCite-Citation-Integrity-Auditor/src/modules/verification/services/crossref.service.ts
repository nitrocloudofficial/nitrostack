...
import type { CrossrefPaper } from "../types.js";
...
function extractYear(item: z.infer<typeof CrossrefItemSchema>): number | null {
  const sources = [
...
function extractAuthors(item: z.infer<typeof CrossrefItemSchema>): string[] {
...
function mapItem(item: z.infer<typeof CrossrefItemSchema>): CrossrefPaper {
  return {
...
export class CrossrefService {
   ...
   */
  async lookupPaper(citation: Citation): Promise<CrossrefPaper | null> {
  ...
  private async lookupByDoi(doi: string): Promise<CrossrefPaper | null> {
    try {
      const response = await withRetry(
        () =>
          this.client.get(`/works/${encodeURIComponent(doi)}`),
  ...
  private async lookupByTitle(title: string): Promise<CrossrefPaper | null> {
    try {
      const response = await withRetry(
        () =>
