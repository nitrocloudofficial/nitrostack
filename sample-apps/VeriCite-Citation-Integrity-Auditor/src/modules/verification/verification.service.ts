...
// Internal helper metadata structure for combining provider responses
interface VerificationMetadataInternal {
  ...
  semanticScholarVerified: boolean;
 ...
 */
function computeExistenceConfidence(
  crossref: CrossrefPaper | null,
  openAlex: OpenAlexPaper | null,
  semScholar: SemanticScholarPaper | null
): number {
  const hits = [crossref, openAlex, semScholar].filter(
    (p): p is CrossrefPaper | OpenAlexPaper | SemanticScholarPaper => p !== null
  );

  const hitCount = hits.length;
  if (hitCount === 0) return 0.0;

  const hasDoiMatch = hits.some(
    (p) => typeof p.doi === "string" && p.doi.trim().length > 0
  );
...
function mergeMetadata(
  ...
  openAlex: OpenAlexPaper | null,
  semScholar: SemanticScholarPaper | null
): VerificationMetadataInternal {
  const resolvedDoi =
    crossref?.doi ?? openAlex?.doi ?? semScholar?.doi ?? null;

  const canonicalTitle =
    crossref?.title ?? openAlex?.title ?? semScholar?.title ?? null;
  ...
  const resolvedAuthors = allAuthorLists.reduce(
    (best, cur) => (cur.length > best.length ? cur : best),
    [] as string[]
  );

  const resolvedYear =
    crossref?.year ?? openAlex?.year ?? semScholar?.year ?? null;

  const abstract = openAlex?.abstract ?? semScholar?.abstract ?? null;

  const citationCount =
    semScholar?.citationCount ?? openAlex?.citationCount ?? null;

  const venue =
    semScholar?.venue ?? openAlex?.venue ?? null;
...
function buildReason(
  exists: boolean,
  existenceConfidence: number,
  supportStatus: VerificationStatus,
  supportReason: string,
  metadata: VerificationMetadataInternal
): string {
  if (!exists) {
  ...
  if (metadata.crossrefVerified) verifiedBy.push("Crossref");
  if (metadata.openAlexVerified) verifiedBy.push("OpenAlex");
  if (metadata.semanticScholarVerified) verifiedBy.push("Semantic Scholar");

  parts.push(
    `Paper confirmed in ${verifiedBy.join(", ")} (existence confidence: ${(existenceConfidence * 100).toFixed(0)}%).`
  );
...
export class VerificationService {
  ...
  private readonly semScholar: SemanticScholarService;
  private readonly supportVerifier: SupportVerifierService;
  ...
  constructor(
    ...
    semScholar?: SemanticScholarService,
    supportVerifier?: SupportVerifierService
  ) {
  ...
  private buildCacheKey(citation: Citation): string {
    if (citation.doi) {
  ...
  private getCached(key: string): EngineVerificationResult | undefined {
  ...
  private setCached(key: string, result: EngineVerificationResult): void {
    this.cache.set(key, {
      result,
  ...
  async verifyCitation(
    ...
    const existenceConfidence = computeExistenceConfidence(
      crossrefResult,
      openAlexResult,
      semScholarResult
    );
    ...
    if (internalMeta.crossrefVerified) verifiedSources.push("Crossref");
    if (internalMeta.openAlexVerified) verifiedSources.push("OpenAlex");
    if (internalMeta.semanticScholarVerified) verifiedSources.push("Semantic Scholar");

    const existence: CitationExistence = exists
    ...
    const supportResult = await this.supportVerifier.verifySupportClaim(
      claim.text,
      internalMeta.abstract
    );
    ...
    const result: EngineVerificationResult = {
      claimId: claim.id,
      citationId: citation.id,
      existence,
  ...
  private async safeLookup<T>(
    ...
    try {
      const result = await withTimeout(
        fn,
        HARD_TIMEOUT_MS,
        `${sourceName} provider lookup timed out`
      );
