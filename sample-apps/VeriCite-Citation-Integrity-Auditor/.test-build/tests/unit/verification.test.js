...
import { toCanonicalResult, toEvidenceRecord, uncitedResult, } from '../../src/modules/verification/verification.adapter.js';
import { politeUserAgent, resolveContactEmail } from '../../src/modules/verification/utils/contact.js';
function claim(text, citationIds = []) {
    return {
...
describe('ContradictionAnalyzer', () => {
    it('detects explicit negation of the claim', () => {
        const a = ContradictionAnalyzer.analyze(claim('Vaccination causes autism in children.'), {
    ...
    it('detects directional polarity conflict', () => {
        const a = ContradictionAnalyzer.analyze(claim('Antibiotic resistance in bacteria decreased across all pathogens.'), {
    ...
    it('treats a retracted source as contradicting regardless of abstract', () => {
        const a = ContradictionAnalyzer.analyze(claim('Vaccination causes autism in children.'), { title: 'Ileal-lymphoid-nodular hyperplasia in children', abstract: 'Association proposed.', retracted: true });
        assert.equal(a.stance, 'CONTRADICTING');
        assert.match(a.reason, /RETRACTED/);
    });
    it('supports a genuinely on-topic abstract', () => {
        const a = ContradictionAnalyzer.analyze(claim('Global surface temperature increased due to greenhouse gas emissions.'), {
    ...
    it('stays NEUTRAL on off-topic evidence rather than guessing', () => {
        const a = ContradictionAnalyzer.analyze(claim('Global surface temperature increased due to greenhouse emissions.'), { title: 'Baroque harpsichord tuning conventions', abstract: 'Discussion of temperament systems.' });
        assert.equal(a.stance, 'NEUTRAL');
    });
    it('does not read an unrelated null finding as refuting the claim', () => {
        const a = ContradictionAnalyzer.analyze(claim('Ice sheet mass loss contributes to sea level rise.'), {
    ...
    it('is deterministic', () => {
        ...
        assert.deepEqual(ContradictionAnalyzer.analyze(claim('Temperature increased due to emissions.'), input), ContradictionAnalyzer.analyze(claim('Temperature increased due to emissions.'), input));
    });
});
...
describe('OfflineVerificationService', () => {
    ...
    const cite = (over = {}) => ({ id: 'cit_1', raw: 'raw', marker: '[1]', resolved: false, ...over });
    it('resolves a fixture by DOI', async () => {
        const r = await offline.verifyCitation(claim('Global temperature increased due to greenhouse gas emissions.'), cite({ doi: '10.1038/s41558-021-01000-0' }));
        assert.equal(r.existence, 'FOUND');
        assert.equal(r.metadata.source, 'Fixture');
        assert.ok(r.evidence);
    });
    it('flags a retracted fixture as CONTRADICTED', async () => {
        const r = await offline.verifyCitation(claim('Vaccination causes autism.'), cite({ doi: '10.1016/S0140-6736(97)11096-0' }));
        assert.equal(r.retracted, true);
        assert.equal(r.status, 'CONTRADICTED');
    });
    it('reports NOT_FOUND for a fake reference rather than inventing a match', async () => {
        const r = await offline.verifyCitation(claim('Quantum entanglement enables faster-than-light communication.'), cite({ doi: '10.9999/not-a-real-doi-99999', raw: 'A Paper That Was Never Written' }));
        assert.equal(r.existence, 'NOT_FOUND');
...
describe('contact / User-Agent construction', () => {
    const withEnv = (value, fn) => {
        ...
        if (value === undefined)
            ...
            process.env['CONTACT_EMAIL'] = value;
        try {
            return fn();
        }
        finally {
            ...
            if (prevAlex !== undefined)
                process.env['OPENALEX_EMAIL'] = prevAlex;
        }
    };
