...
import { DOC_DUPLICATE_CITATIONS, DOC_EMPTY, DOC_MALFORMED_CITATIONS, DOC_UNCITED, } from '../helpers/harness.js';
...
describe('CitationExtractorService', () => {
    ...
    it('parses numbered entries with DOI, year and authors', async () => {
        const parsed = await citations.extractCitations('Body claim about warming.\n\nReferences\n\n'
    ...
    it('ignores ASCII divider rules used as section separators', async () => {
        const parsed = await citations.extractCitations('Body claim here.\n\nReferences\n'
            + '================================================================\n\n'
    ...
    it('links numeric markers, including ranges and lists', async () => {
        const parsed = await citations.extractCitations('Body.\n\nReferences\n\n[1] A. 2001. One.\n[2] B. 2002. Two.\n[3] C. 2003. Three.\n');
    ...
    it('reuses one inferred citation across repeated author-year markers', () => {
        ...
        const mk = (id) => ({
            id, text: `Claim ${id} ${marker}.`, category: 'factual',
            extractionConfidence: 1, context: '', paragraphIndex: 0,
    ...
    it('findInlineMarkers detects both citation styles', () => {
        const markers = CitationExtractorService.findInlineMarkers('Reported widely [12] and elsewhere (Smith et al., 2020).');
        assert.ok(markers.includes('[12]'));
        assert.ok(markers.some((m) => m.includes('Smith')));
    });
});
//# sourceMappingURL=citation-extractor.test.js.map