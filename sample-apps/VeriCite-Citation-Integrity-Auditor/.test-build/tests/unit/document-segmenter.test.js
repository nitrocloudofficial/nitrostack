...
import { extractBodySentences, isNonContentBlock, segmentDocument, splitParagraphs, splitSentences, } from '../../src/shared/document-segmenter.js';
describe('document-segmenter', () => {
    describe('segmentDocument', () => {
        ...
        it('splits body from a References section', () => {
            const { body, references } = segmentDocument('A claim about warming.\n\nReferences\n\n[1] Smith 2020. A paper.');
        ...
        it('stops the body at back matter even when it precedes the references', () => {
            const { body, references } = segmentDocument('Body claim.\n\nAcknowledgements\n\nWe thank people.\n\nReferences\n\n[1] Entry here.');
        ...
        it('does not treat an inline mention of references as a heading', () => {
            const { references } = segmentDocument('We compared our references with prior work in the field.');
    ...
    describe('isNonContentBlock', () => {
        ...
        it('accepts ordinary prose', () => {
            assert.equal(isNonContentBlock('Global mean surface temperature has increased substantially.'), false);
        });
    });
    describe('splitSentences', () => {
        it('does not split on common abbreviations', () => {
            const sentences = splitSentences('Vaswani et al. showed strong results. The effect held e.g. in translation.');
    ...
    describe('extractBodySentences', () => {
        it('reports the paragraph each sentence came from', () => {
            const sentences = extractBodySentences('First claim sentence here. Second in same paragraph.\n\nThird claim in a new paragraph.');
...
//# sourceMappingURL=document-segmenter.test.js.map