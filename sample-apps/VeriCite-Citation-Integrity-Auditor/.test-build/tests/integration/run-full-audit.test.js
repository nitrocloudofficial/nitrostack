...
import { DOC_BIBLIOGRAPHY_ONLY, DOC_CONTRADICTORY, DOC_DUPLICATE_CITATIONS, DOC_EMPTY, DOC_FAKE_REFERENCES, DOC_MALFORMED_CITATIONS, DOC_MIXED_SECTIONS, DOC_RETRACTED, DOC_UNCITED, ExplodingVerificationService, HangingVerificationService, audit, forceOffline, makeAuditTools, makeContext, makeLargeDocument, } from '../helpers/harness.js';
before(forceOffline);
describe('run_full_audit', () => {
        ...
        it('never throws to the MCP layer', async () => {
            for (const input of [undefined, null, 42, {}, []]) {
                await assert.doesNotReject(async () => {
                    ...
                    await makeAuditTools().runFullAudit({ document: input }, ctx);
                });
            }
        });
    });
    ...
    describe('report invariants', () => {
        ...
        it('propagates a caller-supplied correlationId', async () => {
            const { ctx } = makeContext();
            const report = await makeAuditTools().runFullAudit({ document: DOC_CONTRADICTORY, documentName: 'x', correlationId: 'trace-123' }, ctx);
    ...
    describe('MCP tool surfaces', () => {
        it('extract_claims returns the documented shape', async () => {
            const { ctx } = makeContext();
            const out = await makeAuditTools().extractClaims({ text: 'Accuracy improved by 40 percent over the baseline [1].' }, ctx);
        ...
        it('extract_claims reports errors instead of throwing', async () => {
            const { ctx } = makeContext();
            const out = await makeAuditTools().extractClaims({ text: undefined }, ctx);
        ...
        it('verify_citation honours includeEvidence:false', async () => {
            const { ctx } = makeContext();
            const out = await makeAuditTools().verifyCitation({ claim: 'A claim about warming trends.', includeEvidence: false }, ctx);
            assert.equal(out['supportingEvidence'], undefined);
        });
    });
});
//# sourceMappingURL=run-full-audit.test.js.map