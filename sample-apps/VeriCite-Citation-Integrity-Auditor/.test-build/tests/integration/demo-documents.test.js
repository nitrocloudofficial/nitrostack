...
describe('demo documents (Test cases/)', () => {
    ...
    for (const doc of documents) {
        ...
        it(`extracts no bibliography phantoms from "${doc.name}"`, async () => {
            ...
            for (const claim of report.claims) {
                assert.doesNotMatch(claim.text, /^\[\d+\]/, `claim begins with a reference marker: ${claim.text.slice(0, 60)}`);
                assert.doesNotMatch(claim.text, /^[=\-_]{3,}/, `claim is a divider rule: ${claim.text.slice(0, 60)}`);
            }
        });
    }
    it('produces distinguishable outcomes across the corpus', async () => {
        for (const doc of documents) {
            if (!reports.has(doc.name))
                reports.set(doc.name, await audit(doc.text, doc.name));
        }
        const fingerprints = [...reports.values()].map((r) => `${r.integrityScore}|${r.severity}|${r.verdict.level}|${r.summary.totalClaims}`);
        assert.equal(new Set(fingerprints).size, fingerprints.length, `demo outcomes are not distinct:\n${fingerprints.join('\n')}`);
    });
    it('links claims to citations wherever the document cites anything', async () => {
        for (const doc of documents) {
            ...
            assert.ok(linked.length > 0, `"${doc.name}": ${withMarkers.length} claims carry markers but none linked to a citation`);
        }
    });
});
//# sourceMappingURL=demo-documents.test.js.map