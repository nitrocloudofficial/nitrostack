...
import { EVIDENCE_FIXTURES } from '../audit/evidence-fixtures.js';
let OfflineVerificationService = class OfflineVerificationService {
    async verifyCitation(claim, citation) {
        ...
        if (!fixture) {
            return {
                ...
                confidence: 0,
                reason: 'Offline mode: the cited reference matched no record in the local '
    ...
    match(citation) {
        if (citation.doi) {
            const byDoi = EVIDENCE_FIXTURES.find((f) => f.doi.toLowerCase() === citation.doi?.toLowerCase());
            if (byDoi)
                return byDoi;
        }
        const haystack = `${citation.title ?? ''} ${citation.raw}`.toLowerCase();
        const terms = new Set(haystack.replace(/[^a-z0-9\s]/g, ' ').split(/\s+/).filter((t) => t.length > 3));
        return EVIDENCE_FIXTURES.find((f) => f.keywords.some((k) => terms.has(k)));
    }
};
OfflineVerificationService = __decorate([
    Injectable()
], OfflineVerificationService);
export { OfflineVerificationService };
//# sourceMappingURL=offline-verification.service.js.map