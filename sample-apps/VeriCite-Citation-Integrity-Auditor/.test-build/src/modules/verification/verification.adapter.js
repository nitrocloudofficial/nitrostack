...
const EXISTENCE_PRECEDENCE = [
    'FOUND',
    'AMBIGUOUS',
    'NOT_FOUND',
];
function stanceFor(status) {
    if (status === 'SUPPORTED')
        return 'SUPPORTING';
...
export function toEvidenceRecord(result) {
    return {
 ...
 */
export function toCanonicalResult(claim, results) {
    if (results.length === 0) {
 ...
 */
export function uncitedResult(claim) {
    return {
        claimId: claim.id,
        citationIds: [],
        existence: 'NOT_FOUND',
        status: 'UNRELATED',
        confidence: 0,
        reason: 'This claim carries no citation marker linked to the reference list, '
...
export function applyResolution(citation, result) {
...
//# sourceMappingURL=verification.adapter.js.map