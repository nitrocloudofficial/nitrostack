...
const FIELDS = "paperId,title,abstract,authors,year,externalIds,citationCount,venue,publicationVenue";
// ── Zod schemas ───────────────────────────────────────────────────────────
...
function mapPaper(paper) {
    ...
    const venue = paper.publicationVenue?.name ?? (paper.venue || null);
    return {
...
export class SemanticScholarService {
    ...
    constructor() {
        ...
        if (!apiKey) {
            logger.warn("SEMANTIC_SCHOLAR_API_KEY not set — using unauthenticated tier (lower rate limits)");
        }
    }
    /**
    ...
    async lookupByDoi(doi) {
        try {
            ...
            const response = await withRetry(() => this.client.get(`/paper/DOI:${encodeURIComponent(doi)}`, {
    ...
    async lookupByTitle(title) {
        try {
            const response = await withRetry(() => this.client.get("/paper/search", {
                params: {
                    query: title,
                    limit: 3,
                    fields: FIELDS,
