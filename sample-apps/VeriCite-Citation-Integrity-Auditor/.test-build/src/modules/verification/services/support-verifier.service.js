...
export class SupportVerifierService {
     ...
     */
    async verifySupportClaim(claim, abstract) {
        ...
        if (!this.apiKey) {
            logger.warn("Support verifier: GROQ_API_KEY is not set — skipping LLM support check. " +
                "Obtain a free key at https://console.groq.com");
            return fallbackResult("GROQ_API_KEY is not configured; claim support could not be verified.");
        }
        const abstractText = abstract?.trim() ?? "";
        if (!abstractText) {
            logger.warn("Support verifier: no abstract available — skipping LLM call");
            return fallbackResult("Abstract was not available; cannot determine if the claim is supported.");
        }
        const systemPrompt = buildSystemPrompt();
        const userPrompt = buildUserPrompt(claim, abstractText);
        try {
            const rawContent = await withRetry(() => this.callGroq(systemPrompt, userPrompt), { maxAttempts: 3, isRetryable: isAxiosRetryable });
            ...
            return fallbackResult("LLM support verification failed due to an API error.");
        }
    }
    // ── Private methods ─────────────────────────────────────────────────────
    ...
    parseResponse(rawContent) {
        ...
        if (!validated.success) {
            ...
            const rawStatus = String(anyParsed["status"] ?? "").toUpperCase();
...
//# sourceMappingURL=support-verifier.service.js.map