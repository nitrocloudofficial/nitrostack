...
export class ContradictionAnalyzer {
     ...
     */
    static analyze(claim, candidate) {
     ...
     */
    static computeRelevance(claimText, title, abstract) {
        ...
        for (const term of claimTerms) {
            if (titleTerms.has(term))
                score += 2;
            else if (abstractTerms.has(term))
                score += 1;
        }
     ...
     */
    static findNegationCue(abstract, claimText) {
        const claimTerms = ContradictionAnalyzer.contentTerms(claimText);
        for (const cue of NEGATION_CUES) {
            ...
            const window = abstract.slice(Math.max(0, at - NEGATION_LOOKBEHIND), Math.min(abstract.length, at + cue.length + NEGATION_LOOKAHEAD));
            const windowTerms = ContradictionAnalyzer.contentTerms(window);
            let shared = 0;
            for (const term of claimTerms) {
                if (windowTerms.has(term))
                    shared++;
            }
            if (shared >= 2)
                return cue;
    ...
    static findPolarityConflict(claimText, corpus) {
    ...
    static findPercentageConflict(claimText, abstract) {
        ...
        if (claimValues.length === 0 || sourceValues.length === 0)
            return null;
        for (const claimValue of claimValues) {
            // Agreement with ANY reported figure clears the claim.
            const agrees = sourceValues.some((v) => Math.abs(v - claimValue) <= PERCENTAGE_CONFLICT_THRESHOLD);
            if (agrees)
                return null;
        }
        const claimValue = claimValues[0];
        const sourceValue = sourceValues.reduce((closest, v) => Math.abs(v - claimValue) < Math.abs(closest - claimValue) ? v : closest);
...
//# sourceMappingURL=contradiction.analyzer.js.map