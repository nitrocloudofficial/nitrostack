 ...
 */
const INLINE_AUTHOR_YEAR_MARKER = /\(\s*([A-Z][A-Za-z'’\-]+)[^()]{0,60}?[,;\s]\s*((?:1[89]|20)\d{2})[a-z]?\s*\)/g;
let CitationExtractorService = class CitationExtractorService {
    /* ==========================================================
     * Public: parse the reference list
     * ========================================================== */
    async extractCitations(documentText) {
        ...
        return entries.map((entry, index) => this.parseEntry(entry.raw, entry.ordinal ?? index + 1, index));
    }
    /* ==========================================================
     ...
     */
    linkClaimsToCitations(claims, citations) {
    ...
    static findInlineMarkers(sentence) {
     ...
     */
    inferredSearchString(surname, year, claimText) {
    ...
    parseAuthorYear(marker) {
        ...
        if (!surname || !Number.isInteger(year))
            return null;
        return { surname, year };
    }
    authorYearKey(surname, year) {
        return `${surname.toLowerCase()}|${year}`;
    }
    /** Best-effort surname of the first listed author. */
    primarySurname(authors) {
...
CitationExtractorService = __decorate([
    Injectable()
], CitationExtractorService);
export { CitationExtractorService };
//# sourceMappingURL=citation-extractor.service.js.map