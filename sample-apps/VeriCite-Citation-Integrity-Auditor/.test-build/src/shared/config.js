...
function readString(name) {
    ...
    if (cleaned !== raw.trim()) {
        warnings.push(`${name} contained control characters (a CRLF .env file is the usual cause); `
...
function readInt(name, bounds) {
    ...
    if (clamped !== parsed) {
        warnings.push(`${name}=${parsed} is outside [${bounds.min}, ${bounds.max}]; clamped to ${clamped}.`);
    }
    return clamped;
}
function build() {
...
export function validateConfig() {
    ...
    const notices = [];
    if (config.offline) {
        notices.push('VERICITE_OFFLINE=true — evidence is served from recorded fixtures. '
            + 'Results are labelled provider "Fixture" and AuditReport.offlineMode is set.');
    }
    else {
        if (!config.hasGroqKey) {
            notices.push('GROQ_API_KEY is not set — LLM claim-support verification is disabled. '
                + 'Citations still resolve, but every claim degrades to NOT_ENOUGH_EVIDENCE. '
                + 'Get a free key at https://console.groq.com');
        }
        if (!config.hasSemanticScholarKey) {
            notices.push('SEMANTIC_SCHOLAR_API_KEY is not set — the unauthenticated tier is rate '
