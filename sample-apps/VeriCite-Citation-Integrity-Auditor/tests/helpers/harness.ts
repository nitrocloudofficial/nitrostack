...
import type { Citation, Claim, VerificationResult } from '../../src/shared/contracts.js';
...
export function makeContext(): { ctx: ExecutionContext; logs: CapturedLog[] } {
    const logs: CapturedLog[] = [];
    const push = (level: CapturedLog['level']) => (message: string): void => {
...
export function makeAuditTools(
    verifier: CitationVerificationService = new CitationVerificationService(
        new OfflineVerificationService(),
    ),
): AuditTools {
    return new AuditTools(
        new ClaimExtractorService(),
        new CitationExtractorService(),
        verifier,
    );
}
...
export class ExplodingVerificationService extends CitationVerificationService {
    constructor(private readonly failure: Error = new Error('provider exploded')) {
        super(new OfflineVerificationService());
    }

    override async verifyClaimWithResolutions(): Promise<{
...
export function makeLargeDocument(n: number): string {
    const body: string[] = [];
    const refs: string[] = [];
    ...
    for (let i = 1; i <= n; i++) {
        ...
        refs.push(
            `[${i}] Author${i}, A. 20${String(10 + (i % 15)).padStart(2, '0')}. `
...
export async function audit(
    ...
    tools: AuditTools = makeAuditTools(),
) {
    const { ctx } = makeContext();
    return tools.runFullAudit({ document, documentName }, ctx);
}

export type { Claim, Citation };
