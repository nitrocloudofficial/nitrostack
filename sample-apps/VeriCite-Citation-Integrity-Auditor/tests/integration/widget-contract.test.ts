...
describe('widget data contract', () => {
    ...
    it('provides every top-level field the widget reads', async () => {
        ...
        for (const field of readsFromReport) {
            ...
            assert.notEqual(
                (report as unknown as Record<string, unknown>)[field], undefined,
                `AuditReport.${field} undefined`,
            );
        }
    });
    ...
    it('provides every summary KPI card value', async () => {
        ...
        for (const field of readsFromSummary) {
            const value = (report.summary as unknown as Record<string, unknown>)[field];
            assert.equal(typeof value, 'number', `summary.${field} must be a number`);
            assert.ok(Number.isFinite(value as number), `summary.${field} not finite`);
        }
    });
    ...
    it('provides every Trust Verdict panel field', async () => {
        ...
        for (const field of readsFromVerdict) {
            assert.notEqual(
                (report.verdict as unknown as Record<string, unknown>)[field], undefined,
                `verdict.${field} missing`,
            );
        }
        for (const list of ['reasoning', 'strengths', 'weaknesses', 'recommendations'] as const) {
