...
export class PipelineService {
  ...
  async execute(request: PipelineRequest): Promise<PipelineResponse> {
    ...
    try {
      await this.kernel.eventBus.publish('pipeline.started', { requestId });

      // Stage 1: Intent Detection
      const intentResult = this.intentService.detect(request.query);

      // Stage 2-3: Dynamic Agent Creation
      const agents = this.agentFactory.createMany(intentResult.agents);

      // Stage 4: Blackboard (fresh per request)
      const blackboard = new Blackboard();

      // Stage 5: Sequential Agent Execution
      ...
      await this.kernel.eventBus.publish('pipeline.completed', { requestId, executionTime });
