...
class DevUI {
  ...
  showReadyScreen(config: {
    ...
    const lines = [
      chalk.green.bold(`${glyph('✓', 'OK')} Development Server Ready`),
      '',
  ...
  log(message: string, type: 'info' | 'success' | 'warn' | 'error' = 'info'): void {
    const prefix = {
      info: chalk.blue(glyph('ℹ', 'i')),
      success: chalk.green(glyph('✓', 'OK')),
      warn: chalk.yellow(glyph('⚠', '!')),
      error: chalk.red(glyph('✗', 'X')),
    }[type];
      ...
      tscWatchProcess.stdout.on('data', (data) => {
        ...
        if (output.includes('Found 0 errors') || output.includes('Watching for file changes')) {
          if (!tscReady) {
            ...
            console.log(chalk.green(glyph('✓', 'OK')) + chalk.dim(' MCP server recompiled'));
          }
